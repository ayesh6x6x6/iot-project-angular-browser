...
app.get('/getbins', (req, res) => {
    ...
    bins.view('all_bins', 'bins').then(body => {
        body.rows.forEach(doc => {
            //console.log(doc.value.lat);
            //storing values from couchdb in an array
            gotBins.push({ bin_id: doc.value.bin_id, time: doc.value.time, temperature: doc.value.temperature, gas: doc.value.gas, clear: doc.value.clear, lat: doc.value.lat, lng: doc.value.lng });
